...
import {
	...
	MUSIC_CONSERVATIVE_RANGE,
	MUSIC_NO_SPEECH_RANGE,
...
When asking creative questions, use the "Creative Questions" section below for exact wording.
...
- **Text color**: white (${DEFAULT_TEXT_COLOR})
...
Every short needs at minimum: a hook text element in the first 3 seconds, and captions throughout the spoken content. Think about what each text element is doing for the viewer who has their sound off. Hook text should provoke curiosity or state the value proposition — use add_text with a large fontSize and ${DEFAULT_FONT_WEIGHT} fontWeight to make it visually prominent. Add emphasis text for key points (not captions — a standalone add_text element that appears on a punchline or key stat, with a larger fontSize to make it stand out).

${renderSubtitleRules()}

Use generate_captions to automatically create word-timed caption elements synced to the transcript — each caption should be ${MAX_WORDS_PER_CAPTION} words max, positioned at the ${DEFAULT_SUBTITLE_POSITION}, high contrast against the footage, always with scaleX/scaleY ${TEXT_SCALE} and fontSize ${CAPTION_FONT_SIZE}.
...
- Background music: set volume to ${MUSIC_WITH_VOICE_RANGE[0]} to ${MUSIC_WITH_VOICE_RANGE[1]} (barely perceptible, pure atmosphere).
- If in doubt about music level, go lower (${MUSIC_CONSERVATIVE_RANGE[0]} to ${MUSIC_CONSERVATIVE_RANGE[1]}). Too quiet is always better than music that competes with speech.
- Clip audio: mute by default (muted: true). Only the primary voice matters.

**When there is NO speech at all:**
- Music can stay at 0 or go slightly positive (+${MUSIC_NO_SPEECH_RANGE[0]} to +${MUSIC_NO_SPEECH_RANGE[1]}) for energy.
...
**add_text for hook and CTA** — Use ${DEFAULT_FONT_WEIGHT} fontWeight for emphasis text like hooks and CTAs. For ALL text elements: ALWAYS set scaleX/scaleY to ${TEXT_SCALE}. Hook/CTA/emphasis text: fontSize max ${TITLE_FONT_SIZE_MAX}. Caption/subtitle text: fontSize ${CAPTION_FONT_SIZE} (range ${CAPTION_FONT_SIZE_MIN}–${CAPTION_FONT_SIZE_MAX}). NEVER add backgrounds to ANY text — no backgroundColor, no backgroundStyle, no background fill of any kind. Text-only with transparent background, always readable through high contrast color choice against the footage. For captions, prefer generate_captions which handles word-timing automatically.
...
- CTA text should not overlap with caption cards. Stagger them: captions end slightly before CTA begins, or position CTA at center/top while captions are at ${DEFAULT_SUBTITLE_POSITION}.`,
};
