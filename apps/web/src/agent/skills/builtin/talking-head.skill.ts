...
### Step 6: Add captions
Use transcribe_audio to get word-level timing if not already done. Group words into natural caption cards — follow sentence breaks, not fixed word counts. Each card should be readable at a glance (aim for 4–7 words, never more than two lines of thought). Use style 'subtitle', position 'bottom'. Consistent timing: enough to read comfortably, not so long the next sentence has already started.

### Step 7: Supporting elements (if needed)
If the content has distinct sections, consider adding a brief text label at section transitions (style 'label', position 'top') so viewers can follow the structure. Only add these if the content genuinely has navigable sections — don't force structure that isn't there.
...
**add_text for captions** — Never add more words than can be read in the caption's duration. If a sentence is long, split it across two caption cards with a short overlap gap between them.
